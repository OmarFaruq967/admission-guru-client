import React, { useContext } from "react";
import { AuthContext } from "../../Provider/AuthProvider";
import { Link } from "react-router-dom";
import { FaSpinner } from "react-icons/fa";

const AdmissionCard = ({ result, selectedColleges }) => {
  const { user, loading, setLoading } = useContext(AuthContext);
  const { college_name, college_image, college_rating, _id, admission } =
    result;

  // {
  //   const selectedCollege = selectedColleges.find((selectedCollege) => {
  //     selectedCollege?.collegeId;
  //     console.log("selected college Id", selectedCollege?.collegeId);
  //     console.log("college Id", _id);
  //   });
  // }

  // {
  //   const selectedCollege = selectedColleges.find((selectedCollege) => {
  //     const isSelectedCollege = _id === selectedCollege?.collegeId;
  //     console.log("selected college Id", isSelectedCollege);
  //     console.log("college Id", _id);
  //   });
  // }

  const selectedCollege = selectedColleges.find((selectedCollege) => {
    return _id === selectedCollege?.collegeId;
  });

  const isSelectedCollege = !!selectedCollege;

  return (
    <div>
      <div className=" md:h-[350px] bg-[#e2e8f0] border-2 border-r-8 border-b-8 rounded-lg border-[#041838] shadow-xl p-5 ">
        <div>
          <img
            className="h-[100px] md:h-[150px] w-[100%]"
            src={college_image}
            alt=""
          />
        </div>
        <h2 className="text-sm font-medium text-black mt-5">{college_name}</h2>
        <div className="flex mt-2">
          <div>
            <p className="text-sm  text-black  font-medium pr-5 pt-[6px]">
              Rating:
            </p>
          </div>
          <div className="flex items-center mt-2.5 mb-2">
            <svg
              aria-hidden="true"
              className="w-5 h-5 text-yellow-300"
              fill="currentColor"
              viewBox="0 0 20 20"
              xmlns="http://www.w3.org/2000/svg"
            >
              <title>First star</title>
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
            </svg>
            <svg
              aria-hidden="true"
              className="w-5 h-5 text-yellow-300"
              fill="currentColor"
              viewBox="0 0 20 20"
              xmlns="http://www.w3.org/2000/svg"
            >
              <title>Second star</title>
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
            </svg>
            <svg
              aria-hidden="true"
              className="w-5 h-5 text-yellow-300"
              fill="currentColor"
              viewBox="0 0 20 20"
              xmlns="http://www.w3.org/2000/svg"
            >
              <title>Third star</title>
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
            </svg>
            <svg
              aria-hidden="true"
              className="w-5 h-5 text-yellow-300"
              fill="currentColor"
              viewBox="0 0 20 20"
              xmlns="http://www.w3.org/2000/svg"
            >
              <title>Fourth star</title>
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
            </svg>
            <svg
              aria-hidden="true"
              className="w-5 h-5 text-yellow-300"
              fill="currentColor"
              viewBox="0 0 20 20"
              xmlns="http://www.w3.org/2000/svg"
            >
              <title>Fifth star</title>
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
            </svg>
            <span className="bg-blue-100 text-blue-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800 ml-3">
              {college_rating}
            </span>
          </div>
        </div>

        <div className="mt-5">
          {/* <Link to={`/college/${result._id}`}> */}
          {/* <Link to='/admission-form'> */}
          {/* <Link to={`/admission-form?collegeId=${_id}`}> */}
          <Link to={`/get-admission/${result._id}`}>
            {/* <Link to={`/get-admission/${result._id}?collegeId=${selectedCollegeId}`}> */}

            <button
              disabled={isSelectedCollege} // Disable button if collegeId matches result's _id
              className={`bg-[#041838] hover:bg-[#fbbd23] w-full rounded-md py-2 font-medium text-base text-white ${
                isSelectedCollege ? "opacity-50 cursor-not-allowed" : ""
              }`}
              type="submit"
            >
              {loading ? (
                <FaSpinner className="m-auto animate-spin" size={24} />
              ) : (
                "Get Admission"
              )}
            </button>
          </Link>
        </div>
      </div>
    </div>
  );
};

export default AdmissionCard;
